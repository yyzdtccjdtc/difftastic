    ...
    fn new(
        ...
        for (lhs_line_num, rhs_line_num) in line_nums {
            if let Some(lhs_line_num) = lhs_line_num {
                ...
                lhs_max_content = max(
                    lhs_max_content,
                    codepoint_len(lhs_lines[lhs_line_num.0 as usize]),
                );
            }
            if let Some(rhs_line_num) = rhs_line_num {
                ...
                rhs_max_content = max(
                    rhs_max_content,
                    codepoint_len(rhs_lines[rhs_line_num.0 as usize]),
                );
            }
        }
...
fn highlight_as_novel(
    ...
    if let Some(line_num) = line_num {
        ...
        let line_content = lines.get(line_num.0 as usize).map(|s| str::trim(s));
    ...
    for (i, hunk) in hunks.iter().enumerate() {
        ...
        for (lhs_line_num, rhs_line_num) in aligned_lines {
            ...
            if no_lhs_changes && !show_both {
                match rhs_line_num {
                    Some(rhs_line_num) => {
                        let rhs_line = &rhs_colored_lines[rhs_line_num.0 as usize];
                        if same_lines {
                ...
                match lhs_line_num {
                    Some(lhs_line_num) => {
                        let lhs_line = &lhs_colored_lines[lhs_line_num.0 as usize];
                        if same_lines {
                ...
                let lhs_line = match lhs_line_num {
                    Some(lhs_line_num) => split_and_apply(
                        lhs_lines[lhs_line_num.0 as usize],
                ...
                let rhs_line = match rhs_line_num {
                    Some(rhs_line_num) => split_and_apply(
                        rhs_lines[rhs_line_num.0 as usize],
